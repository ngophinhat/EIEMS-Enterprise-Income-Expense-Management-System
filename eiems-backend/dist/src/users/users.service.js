...
let UsersService = class UsersService {
    ...
    async toggleActive(id, currentUserId, currentRole) {
        const user = await this.prisma.user.findUnique({ where: { id } });
        if (!user)
            throw new common_1.NotFoundException('Không tìm thấy tài khoản!');
        if (id === currentUserId) {
            throw new common_1.ForbiddenException('Không thể tắt hoạt động tài khoản của chính mình!');
        }
        if (currentRole === client_1.Role.ADMIN &&
            (user.role === client_1.Role.OWNER || user.role === client_1.Role.ADMIN)) {
